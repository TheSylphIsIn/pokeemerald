...
static void Task_NewGameBirchSpeechSub_InitPokeBall(u8 taskId)
{
    ...
    gSprites[spriteId].y = 66;
    gSprites[spriteId].invisible = FALSE;
    gSprites[spriteId].data[0] = 0;

    CreatePokeballSpriteToReleaseMon(spriteId, gSprites[spriteId].oam.paletteNum, 112, 58, 0, 0, 32, PALETTES_BG, SPECIES_DUBSNAKE);
...
{
    return CreateMonPicSprite_Affine(SPECIES_DUBSNAKE, FALSE, 0, MON_PIC_AFFINE_FRONT, x, y, 14, TAG_NONE);
}
...
static void AddBirchSpeechObjects(u8 taskId)
{
    ...
    lotadSpriteId = NewGameBirchSpeech_CreateLotadSprite(100, 66);
...
void NewGameBirchSpeech_SetDefaultPlayerName(void)
{
    ...
    if (gSaveBlock2Ptr->playerGender == MALE)
        name = gText_DefaultNameMale;
    else
        name = gText_DefaultNameFemale;
...
// Draws player party's icons on the continue window.
static void MainMenu_DrawMonIcons(void)
{
	u32 i;
	u8 spriteId;
