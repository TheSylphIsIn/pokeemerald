/* DO NOT EDIT! THESE FILES ARE OUTPUT BY THE MINDLESS TRAINER EDITOR.MODIFICATIONS WILL BE LOST IF IT IS USED AGAIN. */

static const struct TrainerMon sParty_CalvinFisheyeSet0Choice0[] = {
	{
	.species = SPECIES_SANDUDE,
	.lvl = 7,
	.iv = TRAINER_PARTY_IVS(5, 5, 5, 5, 5, 5),
	.moves = { MOVE_TACKLE, MOVE_HARDEN, MOVE_MUD_SLAP, MOVE_SWIFT, },
	.gender = TRAINER_MON_MALE,
	},
};

static const struct TrainerMon sParty_CalvinFisheyeSet0Choice0Hard[] = {
	{
	.species = SPECIES_SANDUDE,
	.lvl = 7,
	.heldItem = ITEM_ORAN_BERRY,
	.iv = TRAINER_PARTY_IVS(10, 10, 10, 10, 10, 10),
	.moves = { MOVE_TACKLE, MOVE_HARDEN, MOVE_MUD_SLAP, MOVE_SWIFT, },
	.gender = TRAINER_MON_MALE,
	},
};

static const struct TrainerMon sParty_CalvinFisheyeSet0Choice0Unfair[] = {
	{
	.species = SPECIES_SANDUDE,
	.lvl = 8,
	.heldItem = ITEM_BERRY_JUICE,
	.iv = TRAINER_PARTY_IVS(15, 15, 15, 15, 15, 15),
	.moves = { MOVE_TACKLE, MOVE_HARDEN, MOVE_SAND_TOMB, MOVE_SWIFT, },
	.nature = TRAINER_PARTY_NATURE(NATURE_NAIVE),
	.gender = TRAINER_MON_MALE,
	},
};

static const struct TrainerMon sParty_CalvinFisheyeSet0Choice1[] = {
	{
	.species = SPECIES_ELADRIFT,
	.lvl = 7,
	.iv = TRAINER_PARTY_IVS(5, 5, 5, 5, 5, 5),
	.moves = { MOVE_TACKLE, MOVE_GROWL, MOVE_WATER_GUN, MOVE_SWIFT, },
	.gender = TRAINER_MON_MALE,
	},
};

static const struct TrainerMon sParty_CalvinFisheyeSet0Choice1Hard[] = {
	{
	.species = SPECIES_ELADRIFT,
	.lvl = 7,
	.heldItem = ITEM_ORAN_BERRY,
	.iv = TRAINER_PARTY_IVS(10, 10, 10, 10, 10, 10),
	.moves = { MOVE_TACKLE, MOVE_GROWL, MOVE_WATER_GUN, MOVE_SWIFT, },
	.gender = TRAINER_MON_MALE,
	},
};

static const struct TrainerMon sParty_CalvinFisheyeSet0Choice1Unfair[] = {
	{
	.species = SPECIES_ELADRIFT,
	.lvl = 8,
	.heldItem = ITEM_BERRY_JUICE,
	.iv = TRAINER_PARTY_IVS(15, 15, 15, 15, 15, 15),
	.moves = { MOVE_TACKLE, MOVE_GROWL, MOVE_WATER_GUN, MOVE_SWIFT, },
	.nature = TRAINER_PARTY_NATURE(NATURE_SASSY),
	.gender = TRAINER_MON_MALE,
	},
};

static const struct TrainerMon sParty_CalvinFisheyeSet0Choice2[] = {
	{
	.species = SPECIES_GLASMA,
	.lvl = 7,
	.iv = TRAINER_PARTY_IVS(5, 5, 5, 5, 5, 5),
	.moves = { MOVE_TACKLE, MOVE_LEER, MOVE_THUNDER_SHOCK, MOVE_SWIFT, },
	.gender = TRAINER_MON_MALE,
	},
};

static const struct TrainerMon sParty_CalvinFisheyeSet0Choice2Hard[] = {
	{
	.species = SPECIES_GLASMA,
	.lvl = 7,
	.heldItem = ITEM_ORAN_BERRY,
	.iv = TRAINER_PARTY_IVS(10, 10, 10, 10, 10, 10),
	.moves = { MOVE_TACKLE, MOVE_LEER, MOVE_THUNDER_SHOCK, MOVE_SWIFT, },
	.gender = TRAINER_MON_MALE,
	},
};

static const struct TrainerMon sParty_CalvinFisheyeSet0Choice2Unfair[] = {
	{
	.species = SPECIES_GLASMA,
	.lvl = 8,
	.heldItem = ITEM_BERRY_JUICE,
	.iv = TRAINER_PARTY_IVS(15, 15, 15, 15, 15, 15),
	.moves = { MOVE_TACKLE, MOVE_LEER, MOVE_THUNDER_SHOCK, MOVE_SWIFT, },
	.nature = TRAINER_PARTY_NATURE(NATURE_TIMID),
	.gender = TRAINER_MON_MALE,
	},
};

static const struct TrainerMon sParty_MorganFisheyeSet0Choice0[] = {
	{
	.species = SPECIES_ELADRIFT,
	.lvl = 6,
	.iv = TRAINER_PARTY_IVS(10, 10, 10, 10, 10, 10),
	.moves = { MOVE_TACKLE, MOVE_GROWL, MOVE_WATER_GUN, MOVE_NONE, },
	.gender = TRAINER_MON_FEMALE,
	},
};

static const struct TrainerMon sParty_MorganFisheyeSet0Choice0Hard[] = {
	{
	.species = SPECIES_ELADRIFT,
	.lvl = 6,
	.heldItem = ITEM_ORAN_BERRY,
	.iv = TRAINER_PARTY_IVS(15, 15, 15, 15, 15, 15),
	.moves = { MOVE_TACKLE, MOVE_GROWL, MOVE_WATER_GUN, MOVE_FLASH, },
	.gender = TRAINER_MON_FEMALE,
	},
};

static const struct TrainerMon sParty_MorganFisheyeSet0Choice0Unfair[] = {
	{
	.species = SPECIES_ELADRIFT,
	.lvl = 7,
	.heldItem = ITEM_BERRY_JUICE,
	.iv = TRAINER_PARTY_IVS(20, 20, 20, 20, 20, 20),
	.moves = { MOVE_TACKLE, MOVE_GROWL, MOVE_WATER_GUN, MOVE_FLASH, },
	.nature = TRAINER_PARTY_NATURE(NATURE_NAUGHTY),
	.gender = TRAINER_MON_FEMALE,
	},
};

static const struct TrainerMon sParty_MorganFisheyeSet0Choice1[] = {
	{
	.species = SPECIES_GLASMA,
	.lvl = 6,
	.iv = TRAINER_PARTY_IVS(10, 10, 10, 10, 10, 10),
	.moves = { MOVE_TACKLE, MOVE_LEER, MOVE_THUNDER_SHOCK, MOVE_NONE, },
	.gender = TRAINER_MON_FEMALE,
	},
};

static const struct TrainerMon sParty_MorganFisheyeSet0Choice1Hard[] = {
	{
	.species = SPECIES_GLASMA,
	.lvl = 6,
	.heldItem = ITEM_ORAN_BERRY,
	.iv = TRAINER_PARTY_IVS(15, 15, 15, 15, 15, 15),
	.moves = { MOVE_TACKLE, MOVE_LEER, MOVE_THUNDER_SHOCK, MOVE_FLASH, },
	.gender = TRAINER_MON_FEMALE,
	},
};

static const struct TrainerMon sParty_MorganFisheyeSet0Choice1Unfair[] = {
	{
	.species = SPECIES_GLASMA,
	.lvl = 7,
	.heldItem = ITEM_BERRY_JUICE,
	.iv = TRAINER_PARTY_IVS(20, 20, 20, 20, 20, 20),
	.moves = { MOVE_TACKLE, MOVE_LEER, MOVE_THUNDER_SHOCK, MOVE_FLASH, },
	.nature = TRAINER_PARTY_NATURE(NATURE_MILD),
	.gender = TRAINER_MON_FEMALE,
	},
};

static const struct TrainerMon sParty_MorganFisheyeSet0Choice2[] = {
	{
	.species = SPECIES_SANDUDE,
	.lvl = 6,
	.iv = TRAINER_PARTY_IVS(10, 10, 10, 10, 10, 10),
	.moves = { MOVE_TACKLE, MOVE_HARDEN, MOVE_MUD_SLAP, MOVE_NONE, },
	.gender = TRAINER_MON_FEMALE,
	},
};

static const struct TrainerMon sParty_MorganFisheyeSet0Choice2Hard[] = {
	{
	.species = SPECIES_SANDUDE,
	.lvl = 6,
	.heldItem = ITEM_ORAN_BERRY,
	.iv = TRAINER_PARTY_IVS(15, 15, 15, 15, 15, 15),
	.moves = { MOVE_TACKLE, MOVE_HARDEN, MOVE_MUD_SLAP, MOVE_FLASH, },
	.gender = TRAINER_MON_FEMALE,
	},
};

static const struct TrainerMon sParty_MorganFisheyeSet0Choice2Unfair[] = {
	{
	.species = SPECIES_SANDUDE,
	.lvl = 7,
	.heldItem = ITEM_BERRY_JUICE,
	.iv = TRAINER_PARTY_IVS(20, 20, 20, 20, 20, 20),
	.moves = { MOVE_TACKLE, MOVE_HARDEN, MOVE_SAND_TOMB, MOVE_FLASH, },
	.nature = TRAINER_PARTY_NATURE(NATURE_IMPISH),
	.gender = TRAINER_MON_FEMALE,
	},
};

static const struct TrainerMon sParty_MorganCliffClimbSet0Choice0[] = {
	{
	.species = SPECIES_ELEMPTY,
	.lvl = 8,
	.iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
	.gender = TRAINER_MON_MALE,
	.ball = ITEM_GREAT_BALL,
	},
	{
	.species = SPECIES_ELADRIFT,
	.lvl = 11,
	.iv = TRAINER_PARTY_IVS(10, 10, 10, 10, 10, 10),
	.moves = { MOVE_TACKLE, MOVE_GROWL, MOVE_WATER_GUN, MOVE_MEDITATE, },
	.gender = TRAINER_MON_FEMALE,
	},
};

static const struct TrainerMon sParty_MorganCliffClimbSet0Choice0Hard[] = {
	{
	.species = SPECIES_ELEMPTY,
	.lvl = 10,
	.iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
	.gender = TRAINER_MON_MALE,
	.ball = ITEM_GREAT_BALL,
	},
	{
	.species = SPECIES_WOOBAT,
	.lvl = 10,
	.iv = TRAINER_PARTY_IVS(5, 31, 10, 18, 18, 10),
	},
	{
	.species = SPECIES_ELADRIFT,
	.lvl = 11,
	.iv = TRAINER_PARTY_IVS(10, 10, 10, 10, 10, 10),
	.moves = { MOVE_TACKLE, MOVE_GROWL, MOVE_WATER_GUN, MOVE_MEDITATE, },
	.gender = TRAINER_MON_FEMALE,
	},
};

static const struct TrainerMon sParty_MorganCliffClimbSet0Choice0Unfair[] = {
	{
	.species = SPECIES_ELADRIFT,
	.lvl = 7,
	.heldItem = ITEM_BERRY_JUICE,
	.iv = TRAINER_PARTY_IVS(20, 20, 20, 20, 20, 20),
	.moves = { MOVE_TACKLE, MOVE_GROWL, MOVE_WATER_GUN, MOVE_FLASH, },
	.nature = TRAINER_PARTY_NATURE(NATURE_NAUGHTY),
	},
	{
	.species = SPECIES_ELADRIFT,
	.lvl = 7,
	.heldItem = ITEM_BERRY_JUICE,
	.iv = TRAINER_PARTY_IVS(20, 20, 20, 20, 20, 20),
	.moves = { MOVE_TACKLE, MOVE_GROWL, MOVE_WATER_GUN, MOVE_FLASH, },
	.nature = TRAINER_PARTY_NATURE(NATURE_NAUGHTY),
	},
};

static const struct TrainerMon sParty_MorganCliffClimbSet0Choice1[] = {
	{
	.species = SPECIES_ELEMPTY,
	.lvl = 8,
	.iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
	.gender = TRAINER_MON_MALE,
	.ball = ITEM_GREAT_BALL,
	},
	{
	.species = SPECIES_GLASMA,
	.lvl = 11,
	.iv = TRAINER_PARTY_IVS(10, 10, 10, 10, 10, 10),
	.moves = { MOVE_HEADBUTT, MOVE_LEER, MOVE_THUNDER_SHOCK, MOVE_FLASH, },
	.gender = TRAINER_MON_FEMALE,
	},
};

static const struct TrainerMon sParty_MorganCliffClimbSet0Choice1Hard[] = {
	{
	.species = SPECIES_ELEMPTY,
	.lvl = 10,
	.iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
	.gender = TRAINER_MON_MALE,
	.ball = ITEM_GREAT_BALL,
	},
	{
	.species = SPECIES_GLASMA,
	.lvl = 11,
	.iv = TRAINER_PARTY_IVS(10, 10, 10, 10, 10, 10),
	.moves = { MOVE_HEADBUTT, MOVE_LEER, MOVE_THUNDER_SHOCK, MOVE_FLASH, },
	},
	{
	.species = SPECIES_GLASMA,
	.lvl = 11,
	.iv = TRAINER_PARTY_IVS(10, 10, 10, 10, 10, 10),
	.moves = { MOVE_HEADBUTT, MOVE_LEER, MOVE_THUNDER_SHOCK, MOVE_FLASH, },
	.gender = TRAINER_MON_FEMALE,
	},
};

static const struct TrainerMon sParty_MorganCliffClimbSet0Choice1Unfair[] = {
	{
	.species = SPECIES_GLASMA,
	.lvl = 7,
	.heldItem = ITEM_BERRY_JUICE,
	.iv = TRAINER_PARTY_IVS(20, 20, 20, 20, 20, 20),
	.moves = { MOVE_TACKLE, MOVE_LEER, MOVE_THUNDER_SHOCK, MOVE_FLASH, },
	.nature = TRAINER_PARTY_NATURE(NATURE_MILD),
	},
	{
	.species = SPECIES_GLASMA,
	.lvl = 7,
	.heldItem = ITEM_BERRY_JUICE,
	.iv = TRAINER_PARTY_IVS(20, 20, 20, 20, 20, 20),
	.moves = { MOVE_TACKLE, MOVE_LEER, MOVE_THUNDER_SHOCK, MOVE_FLASH, },
	.nature = TRAINER_PARTY_NATURE(NATURE_MILD),
	},
};

static const struct TrainerMon sParty_MorganCliffClimbSet0Choice2[] = {
	{
	.species = SPECIES_ELEMPTY,
	.lvl = 8,
	.iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
	.gender = TRAINER_MON_MALE,
	.ball = ITEM_GREAT_BALL,
	},
	{
	.species = SPECIES_SANDUDE,
	.lvl = 11,
	.iv = TRAINER_PARTY_IVS(10, 10, 10, 10, 10, 10),
	.moves = { MOVE_TACKLE, MOVE_HARDEN, MOVE_MUD_SLAP, MOVE_SAND_TOMB, },
	.gender = TRAINER_MON_FEMALE,
	},
};

static const struct TrainerMon sParty_MorganCliffClimbSet0Choice2Hard[] = {
	{
	.species = SPECIES_ELEMPTY,
	.lvl = 10,
	.iv = TRAINER_PARTY_IVS(12, 12, 12, 12, 12, 12),
	.gender = TRAINER_MON_MALE,
	.ball = ITEM_GREAT_BALL,
	},
	{
	.species = SPECIES_SANDUDE,
	.lvl = 11,
	.iv = TRAINER_PARTY_IVS(10, 10, 10, 10, 10, 10),
	.moves = { MOVE_TACKLE, MOVE_HARDEN, MOVE_MUD_SLAP, MOVE_SAND_TOMB, },
	},
	{
	.species = SPECIES_SANDUDE,
	.lvl = 11,
	.iv = TRAINER_PARTY_IVS(10, 10, 10, 10, 10, 10),
	.moves = { MOVE_TACKLE, MOVE_HARDEN, MOVE_MUD_SLAP, MOVE_SAND_TOMB, },
	.gender = TRAINER_MON_FEMALE,
	},
};

static const struct TrainerMon sParty_MorganCliffClimbSet0Choice2Unfair[] = {
	{
	.species = SPECIES_SANDUDE,
	.lvl = 7,
	.heldItem = ITEM_BERRY_JUICE,
	.iv = TRAINER_PARTY_IVS(20, 20, 20, 20, 20, 20),
	.moves = { MOVE_TACKLE, MOVE_HARDEN, MOVE_SAND_TOMB, MOVE_FLASH, },
	.nature = TRAINER_PARTY_NATURE(NATURE_IMPISH),
	},
	{
	.species = SPECIES_SANDUDE,
	.lvl = 7,
	.heldItem = ITEM_BERRY_JUICE,
	.iv = TRAINER_PARTY_IVS(20, 20, 20, 20, 20, 20),
	.moves = { MOVE_TACKLE, MOVE_HARDEN, MOVE_SAND_TOMB, MOVE_FLASH, },
	.nature = TRAINER_PARTY_NATURE(NATURE_IMPISH),
	},
};

/* DO NOT EDIT! THESE FILES ARE OUTPUT BY THE MINDLESS TRAINER EDITOR.MODIFICATIONS WILL BE LOST IF IT IS USED AGAIN. */

static const struct TrainerMon * const sStarterDependentParties[STARTER_MON_COUNT][STARTER_DEPENDENT_PARTIES_COUNT] = {
	[STARTER_MON_0] = {
		[TRAINER_CALVIN_FISHEYE_LAKE - FIRST_STARTER_DEPENDENT_INDEX] = sParty_CalvinFisheyeSet0Choice0,
		[TRAINER_MORGAN_FISHEYE_LAKE - FIRST_STARTER_DEPENDENT_INDEX] = sParty_MorganFisheyeSet0Choice0,
		[TRAINER_MORGAN_CLIFF_CLIMB - FIRST_STARTER_DEPENDENT_INDEX] = sParty_MorganCliffClimbSet0Choice0,
	},
	[STARTER_MON_1] = {
		[TRAINER_CALVIN_FISHEYE_LAKE - FIRST_STARTER_DEPENDENT_INDEX] = sParty_CalvinFisheyeSet0Choice1,
		[TRAINER_MORGAN_FISHEYE_LAKE - FIRST_STARTER_DEPENDENT_INDEX] = sParty_MorganFisheyeSet0Choice1,
		[TRAINER_MORGAN_CLIFF_CLIMB - FIRST_STARTER_DEPENDENT_INDEX] = sParty_MorganCliffClimbSet0Choice1,
	},
	[STARTER_MON_2] = {
		[TRAINER_CALVIN_FISHEYE_LAKE - FIRST_STARTER_DEPENDENT_INDEX] = sParty_CalvinFisheyeSet0Choice2,
		[TRAINER_MORGAN_FISHEYE_LAKE - FIRST_STARTER_DEPENDENT_INDEX] = sParty_MorganFisheyeSet0Choice2,
		[TRAINER_MORGAN_CLIFF_CLIMB - FIRST_STARTER_DEPENDENT_INDEX] = sParty_MorganCliffClimbSet0Choice2,
	},
};

static const struct TrainerMon * const sStarterDependentPartiesHard[STARTER_MON_COUNT][STARTER_DEPENDENT_PARTIES_COUNT] = {
	[STARTER_MON_0] = {
		[TRAINER_CALVIN_FISHEYE_LAKE - FIRST_STARTER_DEPENDENT_INDEX] = sParty_CalvinFisheyeSet0Choice0Hard,
		[TRAINER_MORGAN_FISHEYE_LAKE - FIRST_STARTER_DEPENDENT_INDEX] = sParty_MorganFisheyeSet0Choice0Hard,
		[TRAINER_MORGAN_CLIFF_CLIMB - FIRST_STARTER_DEPENDENT_INDEX] = sParty_MorganCliffClimbSet0Choice0Hard,
	},
	[STARTER_MON_1] = {
		[TRAINER_CALVIN_FISHEYE_LAKE - FIRST_STARTER_DEPENDENT_INDEX] = sParty_CalvinFisheyeSet0Choice1Hard,
		[TRAINER_MORGAN_FISHEYE_LAKE - FIRST_STARTER_DEPENDENT_INDEX] = sParty_MorganFisheyeSet0Choice1Hard,
		[TRAINER_MORGAN_CLIFF_CLIMB - FIRST_STARTER_DEPENDENT_INDEX] = sParty_MorganCliffClimbSet0Choice1Hard,
	},
	[STARTER_MON_2] = {
		[TRAINER_CALVIN_FISHEYE_LAKE - FIRST_STARTER_DEPENDENT_INDEX] = sParty_CalvinFisheyeSet0Choice2Hard,
		[TRAINER_MORGAN_FISHEYE_LAKE - FIRST_STARTER_DEPENDENT_INDEX] = sParty_MorganFisheyeSet0Choice2Hard,
		[TRAINER_MORGAN_CLIFF_CLIMB - FIRST_STARTER_DEPENDENT_INDEX] = sParty_MorganCliffClimbSet0Choice2Hard,
	},
};

static const struct TrainerMon * const sStarterDependentPartiesUnfair[STARTER_MON_COUNT][STARTER_DEPENDENT_PARTIES_COUNT] = {
	[STARTER_MON_0] = {
		[TRAINER_CALVIN_FISHEYE_LAKE - FIRST_STARTER_DEPENDENT_INDEX] = sParty_CalvinFisheyeSet0Choice0Unfair,
		[TRAINER_MORGAN_FISHEYE_LAKE - FIRST_STARTER_DEPENDENT_INDEX] = sParty_MorganFisheyeSet0Choice0Unfair,
		[TRAINER_MORGAN_CLIFF_CLIMB - FIRST_STARTER_DEPENDENT_INDEX] = sParty_MorganCliffClimbSet0Choice0Unfair,
	},
	[STARTER_MON_1] = {
		[TRAINER_CALVIN_FISHEYE_LAKE - FIRST_STARTER_DEPENDENT_INDEX] = sParty_CalvinFisheyeSet0Choice1Unfair,
		[TRAINER_MORGAN_FISHEYE_LAKE - FIRST_STARTER_DEPENDENT_INDEX] = sParty_MorganFisheyeSet0Choice1Unfair,
		[TRAINER_MORGAN_CLIFF_CLIMB - FIRST_STARTER_DEPENDENT_INDEX] = sParty_MorganCliffClimbSet0Choice1Unfair,
	},
	[STARTER_MON_2] = {
		[TRAINER_CALVIN_FISHEYE_LAKE - FIRST_STARTER_DEPENDENT_INDEX] = sParty_CalvinFisheyeSet0Choice2Unfair,
		[TRAINER_MORGAN_FISHEYE_LAKE - FIRST_STARTER_DEPENDENT_INDEX] = sParty_MorganFisheyeSet0Choice2Unfair,
		[TRAINER_MORGAN_CLIFF_CLIMB - FIRST_STARTER_DEPENDENT_INDEX] = sParty_MorganCliffClimbSet0Choice2Unfair,
	},
};

static const u8 sStarterDependentPartySizes[3][STARTER_DEPENDENT_PARTIES_COUNT] = {
	[OPTIONS_DIFFICULTY_NORMAL] = {
		[TRAINER_CALVIN_FISHEYE_LAKE - FIRST_STARTER_DEPENDENT_INDEX] = ARRAY_COUNT(sParty_CalvinFisheyeSet0Choice0),
		[TRAINER_MORGAN_FISHEYE_LAKE - FIRST_STARTER_DEPENDENT_INDEX] = ARRAY_COUNT(sParty_MorganFisheyeSet0Choice0),
		[TRAINER_MORGAN_CLIFF_CLIMB - FIRST_STARTER_DEPENDENT_INDEX] = ARRAY_COUNT(sParty_MorganCliffClimbSet0Choice0),
	},
	[OPTIONS_DIFFICULTY_HARD] = {
		[TRAINER_CALVIN_FISHEYE_LAKE - FIRST_STARTER_DEPENDENT_INDEX] = ARRAY_COUNT(sParty_CalvinFisheyeSet0Choice0Hard),
		[TRAINER_MORGAN_FISHEYE_LAKE - FIRST_STARTER_DEPENDENT_INDEX] = ARRAY_COUNT(sParty_MorganFisheyeSet0Choice0Hard),
		[TRAINER_MORGAN_CLIFF_CLIMB - FIRST_STARTER_DEPENDENT_INDEX] = ARRAY_COUNT(sParty_MorganCliffClimbSet0Choice0Hard),
	},
	[OPTIONS_DIFFICULTY_UNFAIR] = {
		[TRAINER_CALVIN_FISHEYE_LAKE - FIRST_STARTER_DEPENDENT_INDEX] = ARRAY_COUNT(sParty_CalvinFisheyeSet0Choice0Unfair),
		[TRAINER_MORGAN_FISHEYE_LAKE - FIRST_STARTER_DEPENDENT_INDEX] = ARRAY_COUNT(sParty_MorganFisheyeSet0Choice0Unfair),
		[TRAINER_MORGAN_CLIFF_CLIMB - FIRST_STARTER_DEPENDENT_INDEX] = ARRAY_COUNT(sParty_MorganCliffClimbSet0Choice0Unfair),
	},
};

