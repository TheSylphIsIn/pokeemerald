...
u8 TrySetCantSelectMoveBattleScript(void)
{
    ...
    if (gDisableStructs[gActiveBattler].tauntTimer != 0 && gBattleMoves[move].category == MOVE_CATEGORY_STATUS)
    {
        gCurrentMove = move;
        if (gBattleTypeFlags & BATTLE_TYPE_PALACE)
        {
    ...
    if (((holdEffect == HOLD_EFFECT_CHOICE_BAND)
		...
		|| (holdEffect ==HOLD_EFFECT_CHOICE_SCARF)) && *choicedMove != MOVE_NONE && *choicedMove != MOVE_UNAVAILABLE && *choicedMove != move)
    {
...
u8 CheckMoveLimitations(u8 battlerId, u8 unusableMoves, u8 check)
{
    ...
    for (i = 0; i < MAX_MON_MOVES; i++)
    {
        ...
        if (gBattleMons[battlerId].moves[i] == gLastMoves[battlerId] && check & MOVE_LIMITATION_TORMENTED && gBattleMons[battlerId].status2 & STATUS2_TORMENT)
            unusableMoves |= gBitTable[i];
        // Taunt
        if (gDisableStructs[battlerId].tauntTimer && check & MOVE_LIMITATION_TAUNT && gBattleMoves[gBattleMons[battlerId].moves[i]].category == MOVE_CATEGORY_STATUS)
            unusableMoves |= gBitTable[i];
        // Imprison
        ...
        if (gDisableStructs[battlerId].encoreTimer && gDisableStructs[battlerId].encoredMove != gBattleMons[battlerId].moves[i])
            unusableMoves |= gBitTable[i];
        // Choice Band
        if (holdEffect == (HOLD_EFFECT_CHOICE_BAND || HOLD_EFFECT_CHOICE_SPECS || HOLD_EFFECT_CHOICE_SCARF) && *choicedMove != MOVE_NONE && *choicedMove != MOVE_UNAVAILABLE && *choicedMove != gBattleMons[battlerId].moves[i])
            unusableMoves |= gBitTable[i];
    }
...
u8 AtkCanceller_UnableToUseMove(void)
{
    ...
    do
    {
        ...
        case CANCELLER_TAUNTED: // taunt
            if (gDisableStructs[gBattlerAttacker].tauntTimer && gBattleMoves[gCurrentMove].category == MOVE_CATEGORY_STATUS)
            {
        ...
        case CANCELLER_PARALYSED: // paralysis
            if ((gBattleMons[gBattlerAttacker].status1 & STATUS1_PARALYSIS) && (Random() % 4) == 0
				&& gBattleMons[gBattlerAttacker].ability != ABILITY_QUICK_FEET)
            {
...
u8 AbilityBattleEffects(u8 caseID, u8 battler, u8 ability, u8 special, u16 moveArg)
{
    ...
    if (!(gBattleTypeFlags & BATTLE_TYPE_SAFARI)) // Why isn't that check done at the beginning?
    {
        ...
        case ABILITYEFFECT_MOVES_BLOCK: // 2
            if (gLastUsedAbility == ABILITY_SOUNDPROOF && AbilityIsActive()
				&& gBattleMoves[move].attribute == MOVE_ATTRIBUTE_SOUND)
				{
        ...
        case ABILITYEFFECT_ON_DAMAGE: // Contact abilities and Color Change
            ...
            case ABILITY_ROUGH_SKIN:
                if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT)
				 ...
				 && (gBattleMons[gBattlerAttacker].ability != ABILITY_IRON_FIST))
                {
