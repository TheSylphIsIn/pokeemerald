...
static void CreateSwapLabels(void);
...
static void LoadPartyMenuWindows(void)
{
    ...
    LoadUserWindowBorderGfx(0, 0x50, BG_PLTT_ID(13));
...
void DisplayPartyMenuStdMessage(u32 stringId)
{
    ...
    if (stringId != PARTY_MSG_NONE)
    {
        ...
        DrawStdFrameWithCustomTileAndPalette(*windowPtr, FALSE, 0x50, 13);
...
static u8 DisplaySelectionWindow(u8 windowType)
{
    ...
    DrawStdFrameWithCustomTileAndPalette(sPartyMenuInternal->windowId[0], FALSE, 0x50, 13);
...
{
    DrawStdFrameWithCustomTileAndPalette(WIN_MSG, FALSE, 0x50, 13);
...
static void PartyMenuDisplayYesNoMenu(void)
{
    CreateYesNoMenu(&sPartyMenuYesNoWindowTemplate, 0x50, 13, 0);
}
    ...
    else
    {
        ...
        else if (sFieldMoveCursorCallbacks[fieldMove].fieldMoveFunc() == TRUE)
        {
            ...
            case FIELD_MOVE_TELEPORT:
                ...
                break;
            case FIELD_MOVE_DIG:
				if (IsPlayerOnBurrowEntrance() && MapHasBurrow())
					StringExpandPlaceholders(gStringVar4, gText_BurrowHere);
				else
				{
...
static UNUSED void SpriteCB_BounceConfirmCancelButton(u8 spriteId, u8 spriteId2, u8 animNum)
{
    if (animNum == 0)
    {
...
void ItemUseCB_AbilityScanner(u8 taskId, TaskFunc task)
{
    ...
    else
    {
		...
		StringCopy(gStringVar2, gAbilitiesInfo[gSpeciesInfo[value].abilities[0]].name);
		if (gSpeciesInfo[value].abilities[1]) // appends ",  <second ability name>" to the string if mon has two abilities
		{
			...
			StringAppend(gStringVar2, gAbilitiesInfo[gSpeciesInfo[value].abilities[1]].name);
		}
		StringCopy(gStringVar3, gAbilitiesInfo[gSpeciesInfo[value].abilities[2]].name);
		if (gSpeciesInfo[value].abilities[2] != gSpeciesInfo[value].abilities[3]) // doesn't display the same name twice
		{
			...
			StringAppend(gStringVar3, gAbilitiesInfo[gSpeciesInfo[value].abilities[3]].name);
		}
...
// Unused
static UNUSED void SwitchAliveMonIntoLeadSlot(void)
{
    u8 i;
    struct Pokemon *mon;
    u8 partyId;
...
// Unused
static UNUSED void ChoosePartyMonByMenuType(u8 menuType)
{
...
static void CreateSwapLabels(void)
{
	CopyToBgTilemapBufferRect_ChangePalette(0, sSelectSwitchLabel_Tilemap, 12, 0, 8, 1, 14);
	CopyToBgTilemapBufferRect_ChangePalette(0, sStartSwitchLabel_Tilemap, 21, 0, 8, 1, 14);
    ScheduleBgCopyTilemapToVram(0);
}
