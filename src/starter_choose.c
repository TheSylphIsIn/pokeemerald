...
void CB2_ChooseStarter(void)
{
	...
	if (FlagGet(FLAG_SYS_ENABLE_STARTER_SETS))
...
static void Task_HandleStarterChooseInput(u8 taskId)
{
	...
	bool32 setsEnabled = FlagGet(FLAG_SYS_ENABLE_STARTER_SETS);
    ...
    else if (setsEnabled && JOY_NEW(DPAD_DOWN) && set > 0)
    {
        gTasks[taskId].tStarterSet--;
        gTasks[taskId].func = Task_MoveStarterChooseCursor;
    }
    else if (setsEnabled && JOY_NEW(DPAD_UP) && set < STARTER_SET_COUNT - 1)
    {
        gTasks[taskId].tStarterSet++;
        gTasks[taskId].func = Task_MoveStarterChooseCursor;
    }
