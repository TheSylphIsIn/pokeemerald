...
static void BagMenu_ItemPrintCallback(u8 windowId, u32 itemIndex, u8 y, u8 listPos)
{
    ...
    if (itemIndex != LIST_CANCEL)
    {
        ...
        if ((gBagPosition.pocket != KEYITEMS_POCKET && ItemId_GetImportance(itemId) == FALSE) || ItemId_GetImportance(itemId) > 2)
        {
            // Print item quantity
...
static void PrintTMHMMoveData(u16 itemId)
{
    ...
    else
    {
		...
		BlitMenuInfoIcon(WIN_TMHM_INFO, gMovesInfo[moveId].category + MENU_INFO_ICON_BALL_BLUE + 1, 34, 0);
